function tf = isice(lati_or_xi,loni_or_yi)
% isice returns logical true wherever there is any grounded Antarctic ice or ice shelves
...
%% Description
% 
% tf = isice(lati,loni) returns true for all geo locations (lati,loni) identified as grounded or floating
% ice in the Mask_Antactica_v2.tif (sic) dataset. 
%  
% tf = isice(xi,yi) returns true for all locations identified as grounded or floating ice in Mask_Antarctica_v1.tif, where
...
%    ice = isice(lat,lon); 
% 
% % Mask-out all non-ice-sheet values (use the tilde for NOT) by setting them to NaN: 
% 
%    FA(~ice) = NaN; 
% 
