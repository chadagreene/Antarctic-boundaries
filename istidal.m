function tf = istidal(lati_or_xi,loni_or_yi)
% istidal returns logical true wherever there is open ocean or floating ice shelves 
% according to MEaSURES Antarctic Boundaries for IPY 2007-2009 from Satellite Radar, Version 1. 
% 
% This function returns the complement of isgrounded. 
% 
...
%% Description
% 
% tf = istidal(lati,loni) returns true for all geo locations (lati,loni) identified as grounded
% ice in the Mask_Antarctica_v1.tif dataset. 
%  
% tf = istidal(xi,yi) returns true for all locations identified as grounded ice in Mask_Antarctica_v1.tif, where
...
% 
% % Use istidal to find out which parts of the dataset are influenced by ocean tides: 
% 
%    salty = istidal(lat,lon); 
% 
% % Mask-out all grounded ice (use the tilde for NOT) by setting it to NaN: 
% 
%    FA(~salty) = NaN; 
% 
% % Plot the open ocean and ice shelf gravity data: 
% 
%    pcolorps(lat,lon,FA)
%    axis tight
%
...
% 
% See also: isice, isgrounded, isopenocean, and isiceshelf. 
...
tf = interp2(x,y,Z,xi,yi,'nearest')<255; 
